const covid19ImpactEstimator = (data) => {
  ...
  const severeImpactSevereCasesByRequestedTime = Math.trunc((15 / 100)
  * severeImpactInfestionsByRequestTime);

  const availableBed = (data.totalHospitalBeds * (35 / 100));
  const impactHospitalBedsByRequestedTime = Math.trunc(availableBed
     - impactSevereCasesByRequestedTime);
  const severeImpactHospitalBedsByRequestedTime = Math.trunc(availableBed
    - severeImpactSevereCasesByRequestedTime);

  const impactCasesForICUByRequestedTime = Math.floor((5 / 100)
   * impactInfestionsByRequestTime);
  const severeImpactCasesForICUByRequestedTime = Math.floor((5 / 100)
  * severeImpactInfestionsByRequestTime);
  const impactCasesForVentilatorsByRequestedTime = Math.floor((2 / 100)
  * impactInfestionsByRequestTime);
  const severeImpactCasesForVentilatorsByRequestedTime = Math.floor((2 / 100)
   * severeImpactInfestionsByRequestTime);
  ...
  const impactDollarsInFlight = Math.trunc((impactInfestionsByRequestTime
    * avgDailyIncomeInUSD * avgDailyIncomePopulation) / days);
  const severeImpactDollarsInFlight = Math.trunc((severeImpactInfestionsByRequestTime
    * avgDailyIncomeInUSD * avgDailyIncomePopulation) / days);
